...
function executeFields(exeContext, parentType, sourceValue, fields) {
  ...
  var finalResults = (0, _keys2.default)(fields).map(function (responseName) {
    var fieldASTs = fields[responseName];
    var result = resolveField(exeContext, parentType, sourceValue, fieldASTs);
    if (!result) {
      return 0;
  ...
  console.log('executeFields', finalResults);

  // If there are no promises, we can just return the object
  if (!containsPromise) {
    return finalResults.reduce(function(sum, v) { return sum + v; }, 0);
  }

  return _promise2.default.all(finalResults).then(function (values) {
    return values.reduce(function(sum, v) { return sum + v; }, 0);
  });
}
...
function getCostFn(fieldDef) {
  var resolveFn = fieldDef.cost;
  if (typeof resolveFn === 'function') {
    return resolveFn;
  }
  if (typeof resolveFn === 'number') {
    return function() { return resolveFn; };
  }
  if (resolveFn == null) {
...
function resolveField(exeContext, parentType, source, fieldASTs) {
  ...
  console.log('resolveField', fieldName);
  ...
  var resolveFn = getCostFn(fieldDef);
...
function completeListValue(exeContext, returnType, fieldASTs, info, result) {
  ...
  var completedItem = completeValueCatchingError(exeContext, itemType, fieldASTs, info, 0);
  console.log('completedItem', completedItem);
  if (isThenable(completedItem)) {
    return completedItem.then(function (cost) {
      return result * Number(cost);
    });
  }
...
function completeAbstractValue(exeContext, returnType, fieldASTs, info, result) {
  // TODO iterate all possible types.
  return 0;
